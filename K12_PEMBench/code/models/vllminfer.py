...
class vllminfer():
    def __init__(self, model_name,client_config,retry_times=100):
        self.client = OpenAI(
    ...
    def step_check(response,step_labels):
        try:
            ...
            if index1!=-1 and index2!=-1:
                try:
                    ...
                    if type(list_str[0][0])==list:
                        list_str=list_str[0]
                    flag='1'
                    if len(list_str)!=len(step_labels):
                        flag=0
    ...
    def __call__(self, messes):
        prompt2gpt = messes["prompt2infer"].replace('<image>','')
        img_bin = messes["img"]
        prompt_img = [{
            'type':'image_url',
            'image_url':{
        ...
        prompt_text = [{'type':'text', 'text': prompt2gpt}]
        ...
        for i in range(self.retry_times):
            try:
                ...
                response=self.step_check(response,messes['step_labels'])   
                if response!='':
                    break
            except Exception as e:
                response = ""
...
if __name__ == '__main__':
    print(vllminfer('qwen25_vl_7b_instruct',{
        'api_key':"EMPTY",
        'base_url':'http://0.0.0.0:80/v1',
